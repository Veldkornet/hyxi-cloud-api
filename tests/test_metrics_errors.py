...
async def test_fetch_device_metrics_request_error(caplog):
    ...
    assert "Error fetching metrics for " in caplog.text and ": Mock client error" in caplog.text
...
async def test_fetch_device_metrics_network_error(caplog):
    ...
    assert "Error fetching metrics for " in caplog.text and ": Connection reset" in caplog.text
    # Ensure it didn't crash and entry was not updated with metrics
    assert not entry["metrics"]
...
async def test_fetch_device_metrics_api_error(caplog):
    ...
    assert "HYXI API metrics rejected for " in caplog.text and ": Device not found" in caplog.text
    assert not entry["metrics"]
...
async def test_query_ems_basic_details_error(caplog):
    ...
    assert "HYXI EMS Basic Data Request Failed for " in caplog.text and ": EMS query failed" in caplog.text
...
async def test_query_ems_basic_details_network_error(caplog):
    ...
    assert "HYXI EMS Basic Data Request Failed for " in caplog.text and ": Connection reset" in caplog.text
...
async def test_query_ems_basic_details_non_zero_code(caplog):
    ...
    assert result == {}
    assert "HYXI EMS query returned non-zero code for " in caplog.text and ": 1001" in caplog.text
